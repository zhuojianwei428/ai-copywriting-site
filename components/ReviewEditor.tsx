...
import { DISCLAIMER_LINE, downloadPDF } from "../lib/export";
...
export default function ReviewEditor() {
  ...
  const [editHintOpen, setEditHintOpen] = useState(true);
  ...
  return (
    <main className="min-h-screen bg-surface-canvas">
      ...
      <div className="mx-auto w-full max-w-4xl px-lg py-xl">
        ...
        {doc && (
          <>
            {/* 二次编辑提示：生成后落地到此页，明确告知用户每个部分都可点开修改 */}
            {editHintOpen && (
              <div
                className="mb-md flex items-start gap-sm rounded-lg border px-md py-md"
                style={{
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-title-sm text-title-sm text-text-primary">
                    This draft is fully editable
                  </p>
                  <p className="font-body-sm text-body-sm text-text-muted mt-xs">
                    Click any text below to edit it. Your changes save
                    automatically — polish everything, then export your PDF.
                  </p>
                </div>
                <button
                  onClick={() => setEditHintOpen(false)}
